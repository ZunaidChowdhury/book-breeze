    ...
    return (
            <Link to={`/books/${book.bookId}`} className='p-6 border border-zinc-200 rounded-2xl flex flex-col justify-between group hover:shadow-lg transition-all duration-300 hover:-translate-y-3'>
                <div >
                    ...
                    <h3 className='mt-4 text-text-primary text-2xl font-semibold group-hover:text-theme-primary transition-colors duration-300'>{book.bookName}</h3>
                    <p className='mt-2 text-text-secondary text-xl'>By: {book.author}</p>
                </div>
                ...
                <div className='mt-4 '>
                    ...
                    <div className='my-6 flex gap-2'>
                        {
                            book.tags.map((tag, i) => (
                                <span key={i} className='bg-zinc-100 rounded-full px-4 py-1 text-lg text-text-secondary'>{tag}</span>
                            ))
                        }
                    </div>
